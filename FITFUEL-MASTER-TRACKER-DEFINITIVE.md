...
**Anthropic API key: still not provisioned — and NO LONGER BLOCKING (#216).**
The coach runs on Google Gemini's free tier (no billing, no card). `lib/ai-trainer/
providers/` picks Claude when `CLAUDE_API_KEY` is real and Gemini otherwise, so a
paid key is an upgrade, not a prerequisite.
...
| AI trainer — chat (LLM) | **LIVE on free tier (#216)** | 100 — answers verified |
...
### BLOCK C — AI TRAINER (headline moat; **RUNNING** since 2026-08-18, #216)
...
| F19 ◐(#216) | AI-3 — **DONE and ANSWERING:** `/dashboard/trainer` streaming chat, NDJSON, thread persistence, ownership-checked resume, live on gemini-3.6-flash. **LEFT:** tool use (12D), Luxury gate, 12-SUPPORT, workout-mode. Not yet gated by tier — currently any signed-in user. | data depth | Phase 12 v1 |
...
- `Aug 21, 2026` — **The condensed face is retired everywhere, including the half I had just excused (Decision #223):** #222 closed with "what remains on `--ff-cond` is `/admin` and `/dashboard` only — the app half, which AGENTS.md explicitly allows to differ". **That reading was too generous and this entry reverses it.** AGENTS.md licenses a difference in DENSITY ("density that is correct in a food diary is hostile on a storefront"); the 2026-08-12 verdict that killed uppercase condensed everything was about PARSEABILITY and was never scoped to marketing. A signed-in customer reading their food diary is the same person. **(1) THE COUNT WAS ALSO WRONG.** #222 audited 31 of the app's **60** routes. The 29 unaudited ones were every `/dashboard/*`, every `/admin/*`, plus `/onboarding`, `/order/confirmation`, `/p/[code]`, `/checkout/digital`, `/partners/apply` and `/driver/[token]` — several of them customer-facing, and `/p/[code]` is a stranger's first impression of FitFuel. **(2) BOTH ERROR BOUNDARIES SHIPPED THE REJECTED REGISTER ON EVERY ROUTE**, the same way `app/not-found.tsx` did in #222 and for the same reason its own comment gave. `app/error.tsx` and `app/global-error.tsx` were `--ff-cond` 900, UPPERCASE, up to 96px, `border-radius: 0`. `global-error.tsx` is a special case and now says so at length: next/font never ran by the time it renders, and `var(--font-newsreader), Georgia, serif` does NOT degrade to Georgia — an undefined custom property inside `var()` with no fallback invalidates the whole declaration, so the family is dropped entirely. It is the one file on the site that names its families literally, because there a token is the thing that fails. **(3) `app/_app/theme.ts` WAS THE THIRD LEVER**, after `_ui/theme.ts` (24 files, #222) and `_home/theme.ts` (1 file, #222). It feeds **25** logged-in screens and every helper in it was condensed 800/900 UPPERCASE with radius 0. Migrated whole: `screen()` and `section()` to Newsreader 600 sentence case at **24px and 17px** — the app scale is deliberately kept, the marketing hero is 52px and must not appear on a dashboard; `figure()` from condensed 900 to **MONO**, matching `num()` directly above it, because a column of reported numbers has to line up and lining up is the one thing condensed 900 could not do; `label()` from mono/0.18em to sans/0.12em; `PANEL`, `PANEL_LIVE` and `grid()` given the radius scale (`grid()` also needs `overflow: hidden`, or a square cell pokes out through the container's rounded corner). `COND` survives as a **deprecated alias resolving to the serif**, so no straggling import can reintroduce the face. **(4) A TOKEN SWEEP ALONE WOULD HAVE MISSED SIX FILES.** `/dashboard/body-metrics` (×5), `/admin/supplements` and `components/BeforeAfter` name `'Barlow Condensed'` as a literal string rather than going through `--ff-cond`. Each needed a judgement per site, not a blanket swap: a HEADING goes to the serif at 600 sentence case, a FIGURE goes to mono — body-metrics had both, three of its five hits being reported numbers. **(5) `components/` WAS OUTSIDE THE `app/` SWEEP AND `Footer.tsx` IS ON EVERY PAGE**, carrying a 40px condensed-900 trial price and a `Start trial` CTA at condensed 900 UPPERCASE with an explicit `borderRadius: 0`. Also `WhatsAppOrder` (the floating order button) and `BeforeAfter`. Verified rendered: the CTA is now Archivo 600 sentence case, 8px radius, 51px tall; the price is JetBrains Mono 34px. **VERIFIED:** `grep` for `--ff-cond`, `--font-barlow-condensed` and the literal `Barlow Condensed` across `app/`, `lib/` and `components/` returns **nothing outside `globals.css` and `layout.tsx`**, where the token and the next/font declaration still live; `tsc` clean; `next build` clean; `eslint` **395 problems (338 errors, 57 warnings) before and after, identical**, confirmed by stashing. **NOT VERIFIED AND SAID SO:** every `/dashboard/*` and `/admin/*` route returns **307 to signin** when signed out, so the 25 screens moved by lever (3) are checked at source and by build only — nobody has looked at a rendered dashboard. That is the one open item from this pass.
- `Aug 21, 2026` — **Every remaining customer-facing page onto the homepage system (Decision #222):** Standing instruction was "do checkout now and dont stop dont ask me untill every page has done", so this is the whole tail of the migration in one pass. **(1) `/checkout`** carried `borderRadius: 0` throughout and an UPPERCASE "Complete Your Order"; both gone, and the no-plan early-return branch was missing `className="fk"` entirely, so that branch alone was still hitting the global radius ban. **(2) THE LAST CONDENSED TYPE** on the site was two inline `--font-barlow-condensed` styles, on `/blog` and `/testimonials`. The testimonials one set a **customer's name** in uppercase serif 800 — which is the rejected register and also mangles "D'Souza". **(3) `app/_home/theme.ts` WAS A THIRD THEME FILE**, still Barlow Condensed 900 UPPERCASE lineHeight .83, with exactly one consumer left: `/partners`. Migrated to Newsreader 600 sentence case, and `tag()` from 12.5px/0.28em to 11.5px/0.12em. **The first attempt used the `calc()` scale device from `app/_ui/theme.ts` and that was wrong here** — a blanket multiplier fixes the ceiling and destroys the floor: it took the 375px hero to **22px** and h3s to **14px**, smaller than the body copy under them. That file has 24 consumers and earns the multiplier; this one has one consumer, so its nine call sites are retuned in place instead and both ends of every clamp are now a size somebody chose. **(4) THE `.fk` SPECIFICITY TRAP, MEASURED AT LAST.** `app/_design/base.css` sets `.fk h1, .fk h2, .fk h3, .fk h4` — specificity (0,1,1). `.h1`, `.bandTitle` and `.clauseTitle` in `app/_ui/page.module.css` are (0,1,0), so **every one of those rules was being silently discarded on all 24 pages that use them**. The symptom was visible and had been shipping: `/about`, `/supplements`, `/testimonials` and `/blog` rendered their hero at Newsreader **500/80px** (the raw `--fk-t-4xl` default) while `/plans` and `/` rendered **600/52px** from an inline style that happened to outrank the cascade. Same design system, two different heroes. Fixed by doubling the class (`.h1.h1` → (0,2,0)) rather than `!important` or editing base.css, whose defaults stay correct for any unclassed heading. **(5) CORNER RADIUS RESTORED.** Both `_ui` modules were authored under the old system's hard radius ban, which AGENTS.md retired. Measured on the live pages: `/` had **380** rounded elements and `/about`, `/supplements`, `/testimonials` and `/partners` had **one between them**. Sixteen shared surfaces now take the homepage's own scale — `--fk-r-lg` on cards and wells, `--fk-r` on controls and rows, `--fk-r-full` on status chips — values read off `app/_web/*.css`, not invented. `/about` went 1 → 37. **(6) WHAT WAS CHECKED AND LEFT ALONE.** `/partners` still measures 1 rounded element and that is **correct**: it has no cards, only full-bleed sections divided by hairlines, and rounding a hairline is not a fix. The duplicate `<h1>` the browser reports on `/about` and `/partners` is a dev-overlay artifact — the served HTML has exactly one on every route, verified by counting `<h1` in `curl` output. **(7) THE TWO HIGHEST-VOLUME PAGE TYPES WERE BOTH WRONG, and only a render-level check found them.** All **48 dish pages** hit the same `.fk` trap as (4) — `.title` is (0,1,0) in `app/menu/[dish]/dish.module.css`, so every dish name rendered at the raw **80px** default instead of the `--fk-t-2xl` it asks for. All **59 plan pages** carried a final-CTA h2 at **72px/800**, larger and heavier than any page title on the site. Both fixed at the one shared file each. `/why` had a section h2 at **86px** from `app/_shop/V2Sections.tsx` and another at **77px** from `app/_hp/v2/Sections.tsx`; both now on the `.bandTitle` ramp. That file's `COND` const was also renamed `DISPLAY` — it had been rebound to the serif but kept the name, which is exactly what somebody would re-derive the dead condensed face from. **(8) `/why` SCROLLED SIDEWAYS ON A PHONE** — a 498px document in a 375px viewport, the original complaint that started this whole programme, still live on one route. `.v2-face` in `app/_hp/v2/home.module.css` was a flat **340px** inside a stage measuring 334px, and `.v2-stage` never clipped the 3D drum whose faces swing outside their box by design. The face width is now derived (`min(340px,100%)`, offset `calc(... / -2)`, so the two cannot drift) and the stage takes `overflow-x: clip` — `clip` not `hidden`, so it never becomes a scroll container itself. **(9) THE 404 WAS THE LAST SURFACE IN THE REJECTED SYSTEM**, and its own file comment already said why that mattered: Next ships the not-found boundary in the RSC payload of **every route**, so `--ff-cond` at 900, an UPPERCASE h1, an explicit `border-radius: 0` and eleven hard-coded hex values were being served on top of every migrated page. Rebuilt on the tokens; the 404 figure stays large but moves to **mono**, because every other number on this site is mono and this is a number. **(10) `display()` HAD THE SAME FLOOR BUG THE `_home/theme.ts` ATTEMPT DID — it just took a measurement to see it.** Multiplying the whole clamp by 0.62 is right at the ceiling and wrong at the floor, and the /blog article title was rendering at **22px** on a 375px screen while every other page title on the site sat at 32px; a /plans section h2 was at **18px**. The scale is now wrapped in an outer `clamp(1.75rem, calc(size * 0.62), 3.25rem)` — the same ramp `.h1` and `.bandTitle` use — so the middle term still carries each call site's relative hierarchy but both ends are pinned to the site's own bounds. **(11) `.prose` STYLED p, strong, ul AND li BUT NEVER h2/h3**, so every section heading inside a blog article fell through to `.fk h2` and rendered at **60px — larger than the 52px article title above it**. A section cannot outrank the thing it is a section of. Prose headings added, doubled like the rest so they do not merely tie with base.css and leave the winner to stylesheet order. **(12) THE LAST TWO CUSTOMER-FACING CONDENSED USES**: `/about`'s pull quote (900 UPPERCASE at up to 64px, wider than its own hero) and the related-article headings on `/blog/[slug]`. What remains on `--ff-cond` is **`/admin` and `/dashboard` only** — the app half, which AGENTS.md explicitly allows to differ, and which no customer sees before signing in. **VERIFIED:** 31 routes audited at render level,  zero uppercase or condensed headings anywhere, no heading anywhere above the 52px hero ceiling except two deliberately `sr-only` h1s, **no horizontal overflow at 375 on any of the 31**,  every route carries the `fk` root, every hero is Newsreader 600 52px at 1440 and 32px at 375, no horizontal overflow at 375 on any page, `tsc` clean, `next build` clean, and `eslint` reports the same 22 pre-existing problems before and after (confirmed by stashing).
- `Aug 20, 2026` — **One logo, one way back, and /menu becomes this week's menu (Decision #221):** **(1) THE LOGO WAS FOUR LOGOS** and a customer could meet three in one session: `/` + the 48 dish pages had no mark in Newsreader 21.6/600; every marketing page had a 36px lime square + a bolt icon in sans 20/800; `/dashboard` had a bare lime square in condensed 19/900 **UPPERCASE** (the style AGENTS.md rejects by name); `/auth/signin` had square + bolt with a raw `#84cc16`. A fifth private copy sat in `app/_shop/Chrome.tsx`, and a **sixth** — found only by sweeping after the fact — in `app/opengraph-image.tsx`, which is the logo on every WhatsApp/X/Slack share of every page; its own comment justified the bolt as "identical to the Navbar", a claim that had just become false. Now **`components/Wordmark`**: Newsreader, sentence case, lime "Fuel", no mark, owner's call. Colours go through a `--fk-*` → `--ff-*` → literal chain because it renders inside two token systems that do not define each other's names; size goes through `--wm-size` rather than `fontSize` because inline styles beat every class and the app header still shrinks its wordmark below 640px. **A NEW LOGO IS BEING DESIGNED — it lands in exactly two files, `components/Wordmark.tsx` and `app/opengraph-image.tsx` (which cannot import React components, since it renders to PNG). There is no third.** The OG card's TYPEFACE still differs deliberately: next/og cannot read next/font variables and matching Newsreader means shipping a font binary to an edge function on every crawl. **(2) THE WAY BACK WAS THREE PATTERNS.** `/menu/[dish]` x48 had a breadcrumb AND a second CONTRADICTING link at the very bottom inside the FSSAI compliance sentence — "Back to the menu" → `/` while the breadcrumb's own "Menu" → `/menu`. `/plans/[slug]` x59 had an UPPERCASE 12px mono breadcrumb at 0.2em whose current-page node printed the raw slug at the reader ("gout-veg"). `/blog/[slug]` had no breadcrumb, just a lone "← All articles". Now **`components/Breadcrumb`** on all three: sentence case, 14px, the last ancestor carrying the arrow at a 44px target because it IS the back button, separators `aria-hidden` so a screen reader is not read "slash" between every step, and the current page named in words ("Uric Acid / Gout Management"). A breadcrumb rather than `router.back()` because most detail-page traffic arrives cold from search, where `history.back()` leaves the site. **(3) `/menu` WAS A SECOND CATALOGUE** — all 48 à la carte dishes grouped by course with a basket, which is exactly what `/` is and does better. It now answers the question `/` cannot: **this week's menu**, 7 days x 4 meals from `PlanScheduleSlot`, 26 distinct dishes averaging 1,434 kcal a day, all counted off the rows. **THE TWO DATASETS ARE NOT THE SAME LIST** — `lib/menu-alacarte.ts` is the 48 priced singles (still on `/`); the week is the plan's rotation, different recipes entirely, on a 60-day no-repeat cycle and NOT sold singly. Hence no basket and no price on that page; the close points at the plan. One plan of 126 has a seeded schedule so the plan is NAMED rather than implied. Built on `app/_web/app.module.css` — literally the homepage's stylesheet, not a copy, so the two cannot drift. **TWO CSS TRAPS WORTH REMEMBERING:** `.weekName` COLLIDED — the coach band on `/` already owned it for its "WEEK 1" axis label (11px uppercase 0.08em), so every dish name on /menu rendered in caps. One stylesheet shared by two pages makes class names a global namespace. And `.proof` no longer exists (deleted with the six figures the owner removed), so the stat row rendered `class="undefined"`. **(4) CHANGING WHAT /menu MEANS BROKE FOUR LINKS**, plus a same-day sweep of the new page found **3 of its 7 links dead**, all from `app/_web/AppChrome.tsx` which wraps /menu AND all 48 dish pages: `/?mode=plans` and `/?mode=supps` had been linked since AppChrome was written while `app/page.tsx` read no query params at all (`/` now reads and validates `?mode=`), and the SEARCH BOX pushed `/?q=…` which nothing had read since search was removed from `/` on 2026-08-19 — a customer typed a dish name on a dish page, pressed enter, and their search was silently discarded. Search removed from AppChrome; `dishCount`, `router`, `q`, `setQ` orphaned with it and cleaned. The dish pages' breadcrumb parent moved from `/menu` to `/` — a trail to a page that no longer contains the dish. **THE STANDING LESSON, applied unprompted this time:** a 200 is not a working link. Every failure in #220 and #221 returned 200 and did nothing — a query param the destination never reads, an anchor with no target, a breadcrumb to a page without the item. Grep the destination for `?param` names; `getElementById` every `#anchor`; expand every collapsed list before collecting. `app/menu/MenuClient.tsx` is left on disk, no longer imported — it holds the à la carte grid and enquiry wiring, which live only on `/` now. Deleting it is a separate call. tsc/eslint/build clean; no new lint errors (338 repo-wide before and after). Verified rendered at 375 and 1440.
- `Aug 20, 2026` — **The homepage link audit, done properly the second time (Decision #220):** Asked to check every URL end to end before moving off `/`. The FIRST pass was not an audit: it collected 122 hrefs from a partially expanded page and HTTP-tested about a dozen, then reported everything resolved. Challenged on it, the real figure — after expanding every course group and both Show-all controls across all three catalogue modes — is **220 links**, and **four** were broken, not one. **(1) `/order` → 404.** Arrived with the Inside.tsx surfaces directory; `app/order/` holds `confirmation/page.tsx` and `success/route.ts` (a PayU webhook) and nothing else. Not repointed — the surface it describes is `app/dashboard/DeliveryConfirmCard.tsx`, i.e. the “Today” row directly above it, so the row was removed and its tracking claim folded into Today. Nine surfaces that resolve beats ten where one is a duplicate and a 404. **(2) FORTY-SIX DEAD SUPPLEMENT ANCHORS.** Every supplement card links `/supplements#<slug>`; that page carried **three** `id`s in total and none was a supplement — each renders as a `<button>` opening a modal. All 46 dumped the reader at the top of the marketing header. `app/supplements/SupplementsLanding.tsx` line 362 generates the same anchor internally, so it was broken from both directions. `lib/supplements-db.ts` maps `id: r.slug`, so the fix is `id={supp.id}` on the card plus `scroll-margin-top: 96px` to clear the sticky nav. **(3) TWENTY-NINE CONDITION CHIPS ON A PARAM NOTHING READS.** `app/plans/page.tsx` reads `diet` and `trial` and has never read `q`, so every chip landed on the unfiltered list of all 126 plans. Replaced with an explicit label→slug table in `home-data.ts`, every entry checked against the database and every slug verified 200. Six needed a name the label does not spell: Diabetes→`diabetic-veg`, Postpartum→`post-pregnancy-veg`, Cholesterol→`heart-health-veg`, PCOD→`pcos-veg`, Seniors→`senior-veg`, Kids and teens→`kids-teen-veg`. A TABLE and not `slugify()`, which would invent `/plans/kids-and-teens` and 404 silently. **(4) THE PLAN BUILDER CTA carried four params, three ignored** — same root cause, so the URL looked like it handed over the configuration and did not. Now sends only `diet` and the button reads “See vegetarian plans” rather than “Start this plan”. **VERIFIED THIS TIME, EXHAUSTIVELY:** all **48** `/menu/[dish]` and all **59** `/plans/[slug]` tested individually rather than inferred from the build manifest — every one 200; all 46 supplement anchors confirmed present via `getElementById`; 20 marketing + 5 legal + `/checkout` `/menu` `/plans` `/supplements` `/plans/digital` `/order/confirmation` all 200; 11 `/dashboard/*` → 307 to `/auth/signin?callbackUrl=…`, one hop, 200. 8 images referenced, 8 present, none broken, all with `alt`. One `h1`, no heading-level skips, 12 labelled sections, both tables captioned, zero empty links or buttons. **THE LESSON, recorded to memory:** a 200 is not a working link. Both failure modes here returned 200 — a query param the destination never reads, and an anchor with no target. Expand every collapsed list before collecting, test every dynamic instance rather than a sample, grep the destination for `?param` names, and `getElementById` every `#anchor`. **STILL UNVERIFIED and unchanged from #218/#219:** the browser pane has never composited in any session, so `/` has been measured through the DOM and never SEEN rendered; screenshots were impossible throughout and the scroll-driven reveal path is untested (the 4s failsafe fires in place of the IntersectionObserver). HTTP 200 on a destination also means it responded, not that its content is correct — the dashboard pages sit behind auth and were never rendered.
- `Aug 20, 2026` — **THE HOMEPAGE IS CLOSED. Four more v2 imports, and what is deliberately left behind (Decision #219):** Continues #218 at the owner's instruction (“import week rotation and other things you think are best … let's get this homepage a goodbye”). All 23 unrouted `app/_hp/*` components were surveyed against what the twelve bands already carry; most are covered (PriceBuilder/Offers → the plan builder, Counts → the platform band, Conditions → the conditions band, Steps → the close, Film → the day timeline, Corporate/Network → the services trio). Four were not. **(1) THE WEDGE** — `Wedge.tsx`, holding the best line the old page had and burying it: *every app trusts you to log it, we already know*. Seven rows against a tiffin service, a fitness app and a supplement brand, on cooking / weighing / logging / training / body metrics / recalibration / diagnosis. Placed directly after the platform band, because that band says what we run and this one says why the cheaper things do not — answering “why not a tiffin service” below the price is answering it too late. A TABLE and not four cards, every cell a sentence and never a tick (a glyph alone puts a whole row's meaning on a 1.5:1 shape); the two live figures are passed in, not typed. **(2) THE PROOF** — `Proof.tsx`, and this closes the ONE thing #217 dropped: `app/page.tsx` had been querying three featured Testimonial rows and throwing them away for four days because the imported design had no proof band. Result, plan and AREA — the area matters most, because “Kharadi” tells a Kharadi reader this is a real local business, which is the hardest thing for a national brand to fake. Renders NOTHING on an empty table; a placeholder quote would be a lie. **(3) THE WEEK ROTATION** — `Week.tsx` + `menu-data.ts`. `Behind.tsx` has argued “nothing repeats for sixty days” in prose since it was written and no surface ever showed a day of it. Seven real days of **weight-loss-veg**, the one plan of 126 with a seeded `PlanScheduleSlot` set: **26 distinct dishes across 7 days, counted from the rows rather than claimed**, so a repeat in the seed drops the figure. Day one totals **1,430 kcal — the same number the day band prints as “logged for you”**, which was a coincidence until now and is checkable from now on. Renders nothing when the query is empty; empty cells are an em dash, never a plausible dish name. **(4) THE DASHBOARD DIRECTORY** — `Inside.tsx`, closing a loss #217 made quietly: `Platform.tsx` carried EIGHT dashboard screens as rows and the redesign replaced it with four stat columns, so `/` went from naming eleven surfaces to naming none. Ten rows now (Today, the diary, training, body metrics, progress, supplements, referrals, partner console, notifications, order tracking), each with the figure that proves it. Inside's own comment is the argument for the shape: giving each of these a section is exactly how the last homepage reached 39,708px and 35 headings — ten rows cost one screen, ten cards would cost ten. **THE PAGE, FINAL:** twelve bands — the day / the platform / the wedge / the week / the services / the plan builder / the conditions / the coach / the delivery map / the surfaces / the proof / the close. Three tables (wedge, week, and the spec shape) share one sideways-scroll-with-sticky-row-header behaviour. 375x812: header 220px, first dish card at 596px, one card 270px, no horizontal overflow at 375 or 320, every target 44px bar two inline prose links. tsc/eslint/build clean. **DELIBERATELY LEFT IN `app/_hp/` AND NOT AN OVERSIGHT:** `Film`, `More`, `Why`, `Hungry`, `Loop`, `Ticker`, `Wedge`'s neighbours. Those are the scrolling-narrative sections; importing them reassembles the 29-section brochure this page replaced. Anything imported from that directory in future should be rebuilt at app density on `--fk-*` tokens, never remounted — `HomeBands.tsx`, `DeliveryMap.tsx` and `WeekBand` are the worked examples. **VERIFICATION CAVEAT, carried from #218 and still true:** the browser pane never composited across either session, so every figure in #218 and #219 was measured through the DOM and the page has never actually been SEEN rendered. Screenshots were impossible and the scroll-driven reveal path is unverified (the 4s failsafe fired in place of the IntersectionObserver, which is what it exists for). A phone screenshot of `/` is the cheapest possible next check before work moves to other pages.
- `Aug 20, 2026` — **The homepage is responsive, and the best of v2 is finally on a route (Decision #218):** Four rounds after #217, driven by the owner reporting the phone unusable. **(1) THE HEADER.** Measured 427px of sticky header over a 59px fixed tab bar on a 375x812 phone — a 326px window onto the menu, 40% of the screen, 181px on a 667px handset. Cut to 220px: the day bar's basket column goes (the bag badge and the order bar carry that total twice already), the mode row goes (it IS the first three items of the tab bar, fixed on screen 59px below it), and the top row folds to one line. **(2) THE CASCADE BUG, which was the real answer to “still not mobile responsive” and was never a layout problem.** `app/_design/base.css` sets the display scale on the ELEMENT inside the theme root — `.fk h1`, `.fk h2`, `.fk p`, `.fk a` — at **(0,1,1)**. Every heading and paragraph class in `app.module.css` is **(0,1,0)**, so NONE of their declared values had ever shipped. Measured at 1440: the h1 at 80px against its own 32, band headings 60 against 42, and **the footer's four column labels — Order / FitFuel / Proof / Legal — at 60px display type against their own 11px**. On a phone those clamp-floor at 44 and 36. Four properties were being lost this way: font-size, margin, font-weight (every display heading rendering 500 where it declared 600, a step lighter than the `<b>` labels on the cards below it) and colour (`.fk a { color: inherit }` beat `.dishName`, so the most repeated text in the catalogue rendered a step down the ink scale). Fixed by restating each at (0,2,0) with an `.app` prefix — same values, base.css untouched, since a design system is right to set a default scale. **Any future heading/paragraph class in that file needs the same prefix.** **(3) THE DISH CARD was 421px tall at 375px** — one card per screen on a 48-dish menu — with an empty 124px well taking 37% of it, a contribution line wrapping to four lines in a 139px box, and “₹16 per gram of protein” wrapping to FOUR lines of 12px mono in a 69px column. Both figures hidden on the phone, kept on desktop; well to 104px. Card 421→270. **(4) REMOVED on the owner's call:** the search field entirely (with its “/” shortcut and the numeric “30g protein” mode — course chips and the tab bar are what narrow the menu now, and /menu keeps a searchable list), the sort row, the dish-well **monogram** (which closes #217's flagged AGENTS.md conflict in AGENTS.md's favour), the “KHARADI, PUNE” eyebrow, the six figures on the fold, and all seven band eyebrows. Removing the eyebrows is what exposed the paragraph half of the cascade bug — they had been holding open the space the dead margins were supposed to provide. **(5) THREE IMPORTS FROM v2, which the owner asked for by name.** `app/_hp/Finder.tsx`'s **Mifflin St Jeor calculator** — sex/age/height/weight/activity through `lib/tdee.ts`, the same module onboarding and the plan builder run — is now behind the day bar, so the catalogue's arithmetic is the reader's: a 95kg male, 31, 172cm, lightly active gets 2,578 kcal / 152g on maintenance and 2,078 / 190g on a cut, because lib/tdee applies −500 kcal AND raises protein 1.6→2.0 g/kg; the first card's line moves from “10% of your protein” to “9%”. localStorage only, no account, read via `useSyncExternalStore` and validated on read. `app/_hp/Areas.tsx`'s **plotted delivery map** — every suburb projected from its published centroid, 3/6/9km rings — was sheet-only behind the location chip; rebuilt at app density in `app/_web/DeliveryMap.tsx` off the SAME `areas-data.ts`, with the area labels dropped below 640px (the frame renders 309px against a 720-unit viewBox, so they landed at 4.6 effective pixels) and the shape kept. And **the close band**, which the owner called stale and was: headline + paragraph + two buttons under eight bands of evidence. Now three steps (tonight / 04:00 / by 08:00) and the three things people check before paying, each linking to the policy page that governs it. 375x812 across the session: header 427→220px, first dish card 792→590px, one card 421→270px. No horizontal overflow at 375 or 320; every target ≥44px bar inline prose links. tsc/eslint/build clean throughout. **Caveat on verification:** the browser pane never composited this session, so everything was measured through the DOM rather than seen — no screenshot was possible, and the scroll-driven reveal path is unverified (the 4s failsafe fired instead, which is what it exists for).
- `Aug 19, 2026` — **The imported homepage redesign, wired to real data (Decision #217):** `FitFuel Homepage (redesign).dc.html` pulled from the Claude Design project via the design MCP and implemented against the actual app rather than against its own fixtures. **Shell:** the cutoff strip becomes a live chip beside the wordmark (still mode-aware — cooking today / the plan cutoff / researched-not-stocked); a **day bar** joins the sticky header, where Cut/Maintain/Build sets a kcal and protein target and the basket fills two meters against it, every dish card prints its own share of that target, and “Fits your day” marks dishes that still fit what is left; the search box now takes a number (`30g protein`, `under 400 kcal`) because the placeholder offered it; a sort row (menu order / best protein per calorie / most protein / fewest calories), where menu order breaks the grid into the six courses with the kitchen's own note on each — `COURSES.note` has carried that sentence all along and nothing rendered it; the trial moves into a rail card, with the inline row kept for phones where there is no rail and CSS showing exactly one. **Eight bands below the food** (`app/_web/HomeSections.tsx`): the day 04:00→door, the platform, the seven services, a plan builder, the conditions, the coach + trial receipt, drop times + questions, the close. `app/_web/useReveal.ts` carries the entrance system — one IntersectionObserver, four opt-in data attributes, a hard bail under `prefers-reduced-motion` BEFORE anything is hidden, a scrolled-past test and a 4s failsafe. **Every number is the database's, not the design's:** the plan builder prices off the seeded PlanPrice matrix through `lib/plan-tier-pricing` + `lib/pricing-decomposition` — the same two modules /plans and checkout use — and Premium/Luxury, which have no MealPlan rows, price as estimates and say so rather than taking money; the trial receipt is `lib/trial-price` (300 + 50 + 50 + 20 = 420), where the design's own five lines totalled **513 under a heading reading 420**; conditions, goals and every stat are live counts. **Two pre-existing bugs found on the way:** `--fk-max` was read by `.platformInner`, `.behindInner` and `.b2bInner` and defined nowhere, so three bands ran full width on a wide monitor while the header stopped at 1440; and the rail's sticky offset was a typed 24px that put its top item under the filter row at every width. A ResizeObserver (plus a measure on mode change, because plans adds a chip row and RO is a frame late) publishes the header's real height as `--fk-head` and both offsets derive from it. **TWO THINGS FOR THE OWNER TO RULE ON, both flagged in code:** (1) the empty dish well now carries the dish's **initial in a ring** — AGENTS.md bans a typographic stand-in where someone is choosing what to eat, the design draws one; implemented as drawn, one element from being empty again. (2) the design has **no proof band**, so the three featured testimonials are no longer on `/`; the query is left in `getBandData` so re-wiring is one prop. `Platform.tsx`, `Behind.tsx`, `HomeBands.tsx` and `services.ts` are no longer imported by `/` but are untouched on disk. tsc/eslint/build clean; day bar, sort, grouping, numeric search, FAQ, all three catalog modes and the plan arithmetic verified in the running app.
- `Aug 18, 2026` — **The coach actually runs (Decision #216):** #215 shipped the AI with no key and no executed model call. It now answers. `lib/ai-trainer/provider.ts` + `providers/{anthropic,gemini}.ts` make the model a runtime choice: real `CLAUDE_API_KEY` -> Opus 5, else `GEMINI_API_KEY` -> Gemini Flash **free tier (no billing, no card — Google's docs are explicit that billing is only needed to move PAST it)**, else the screen says so. Quality-first order, so a paid key upgrades the coach with no code change. The Claude path moved behind the interface UNCHANGED (same Opus 5, adaptive thinking, cache breakpoint, refusal fallback). **Three bugs found only by making the real call, none findable by reading:** (1) `isConfigured()` demanded an `AIza` prefix, but AI Studio issues `AQ.` keys now — a valid key read as 'not switched on'; fixed to a length check, since enumerating prefixes just relocates the bug to Google's next format change. (2) `gemini-2.5-flash` is listed free but is CLOSED to accounts created recently — a free-tier listing is not availability; default is now `gemini-3.6-flash`, the model Google's own 404 named. (3) 3.x Flash thinks at medium by default and finishes thinking BEFORE the first character streams — 87s of dead air; `thinkingConfig.thinkingLevel: low` brings first word to **3.8s**. `thinkingBudget:0` is rejected — thinking cannot be disabled on 3.x. Verified live: a plateau question answered from real numbers with no invention, and a 'should I stop my thyroid medication' probe refused correctly then continued usefully — 12-SAFE holds. `AiMessage.model` added (nullable, pushed to Neon) so a thread spanning a provider switch stays attributable. tsc/eslint/build clean.
