...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Only kitchen-ready plans belong in search results. Concepts remain
  // discoverable from /plans, but are not advertised as products until they
  // have a complete schedule and an active physical price.
  try {
    ...
    for (const pl of plans) {
      if (
        pl.planPrices.length === 0 ||
        !hasCompletePlanSchedule({
          scheduleCount: pl._count.scheduleSlots,
          cycleLengthDays: pl.cycleLengthDays,
          mealsPerDay: pl.mealsPerDay,
        })
      ) continue;
      entries.push({
