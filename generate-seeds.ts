...
const MissingFoodItemSchema = z.object({
  name: z.string(), key: z.string(), category: z.string(),
  per100Calories: z.number(), per100Protein: z.number(), per100Carbs: z.number(),
  per100Fat: z.number(), per100Fiber: z.number(),
});
const IngredientSchema = z.object({
  foodItemKey: z.string(), quantityGrams: z.number(), cookedWeightFactor: z.number(),
  prepNote: z.string().optional(), isOptional: z.boolean(), orderInRecipe: z.number(),
});
const StepSchema = z.object({
  stepNumber: z.number(), title: z.string(), instruction: z.string(),
  durationMins: z.number().optional(), technique: z.string().optional(), kitchenNote: z.string().optional(),
});
const RecipeSchema = z.object({
  name: z.string(), slug: z.string(), description: z.string(), shortDescription: z.string(),
  cuisineType: z.string(), mealType: z.enum(['BREAKFAST', 'LUNCH', 'DINNER', 'SNACK']),
  dietaryTags: z.array(z.string()), planCategories: z.array(z.string()),
  servingSizeGrams: z.number(), prepTimeMins: z.number(), cookTimeMins: z.number(),
  difficulty: z.string(), equipmentNeeded: z.array(z.string()), allergens: z.array(z.string()),
  shelfLifeHours: z.number(), packagingType: z.string(), kitchenStation: z.string(),
  seasonTags: z.array(z.string()), rotationGroup: z.number(),
  caloriesPerServing: z.number(), proteinGrams: z.number(), carbsGrams: z.number(),
  fatGrams: z.number(), fibreGrams: z.number(), caloriesPer100g: z.number(),
  proteinPer100g: z.number(), carbsPer100g: z.number(), fatPer100g: z.number(), fibrePer100g: z.number(),
  isFeatured: z.boolean(), ingredients: z.array(IngredientSchema), steps: z.array(StepSchema),
});
...
const OUTPUT_DIR = path.join(process.cwd(), 'output'); // ✅ Safe testing folder
const PROGRESS_FILE = path.join(process.cwd(), 'progress.json');
const BATCH_SIZE = 2; // ✅ CRITICAL: Reduced to 2 to force Gold Standard prose & math
const DELAY_BATCH = 2000;
const DELAY_PLAN = 3000;
const MAX_RETRIES = 5; // ✅ Increased retries to handle strict quality gate rejections
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. PLAN REGISTRY (From your generate-all-seeds.js)
// ─────────────────────────────────────────────────────────────────────────────
type PlanConfig = { slug: string; file: string; planCategory: string; dietVariant: string; recipeCount: number; cal: number; protein: number; description: string };
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. STRICT QUALITY GATE & SANITIZER (Rejects Lazy LLM Output)
// ─────────────────────────────────────────────────────────────────────────────
function extractAndSanitizePayload(rawJson: string, requestedSlots: string[]): any {
  ...
  // 🛑 QUALITY GATE 1: Reject if LLM copy-pasted identical macros
  const calories = parsed.recipes.map((r: any) => r.caloriesPerServing || r.CaloriesPerServing);
  const uniqueCalories = new Set(calories);
  if (uniqueCalories.size === 1 && parsed.recipes.length > 1) {
    throw new Error("LLM copy-pasted identical macros for all recipes. Rejecting batch.");
  }

  // 🛑 QUALITY GATE 2: Reject empty steps, UNKNOWN ingredients, and bad meal types
  parsed.recipes.forEach((r: any, idx: number) => {
    const name = r.name || r.slug || `Recipe ${idx+1}`;
    
    // Check Steps
    const steps = r.steps || r.Steps || [];
    if (steps.length === 0) throw new Error(`LLM generated 0 steps for ${name}. Rejecting.`);
    for (const s of steps) {
      const inst = s.instruction || s.Instruction || '';
      if (inst.length < 30) throw new Error(`LLM generated empty/lazy step instruction for ${name}. Rejecting.`);
    }
    ...
    // Check Meal Type against requested slots
  ...
  // Sanitize & Map (Only reached if Quality Gate passes)
  parsed.missingFoodItems = (parsed.missingFoodItems || []).map((item: any) => {
    if (typeof item === 'string') {
      ...
      return { name, key, category: 'Other', per100Calories: 50, per100Protein: 2, per100Carbs: 10, per100Fat: 1, per100Fiber: 2 };
    }
  ...
  parsed.recipes = parsed.recipes.map((r: any, idx: number) => {
    ...
    const name = r.name || r.Name || r.slug.replace(/-/g, ' ');
    ...
    return {
      ...
      proteinGrams: Number(r.proteinGrams),
      carbsGrams: Number(r.carbsGrams),
      fatGrams: Number(r.fatGrams),
      fibreGrams: Number(r.fibreGrams || r.fiberGrams),
      ...
      fibrePer100g: Number(r.fibrePer100g || r.fiberPer100g),
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. CORE LOGIC & PROMPT ENGINEERING
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// 6. CORE LOGIC & PROMPT ENGINEERING (UPDATED WITH MACRO MATH ENFORCEMENT)
// ─────────────────────────────────────────────────────────────────────────────
async function generateBatch(plan: PlanConfig, batchSlots: Record<string, number>, batchIndex: number, totalBatches: number, existingSlugs: Set<string>, knownFIKeys: Set<string>) {
  ...
  const prompt = `
    ...
    4. If an ingredient is NOT in the lists above, add it to "missingFoodItems" with REALISTIC macros (NOT zeros) and use its new EXACT key.
    5. Avoid duplicate slugs. Used slugs: [${Array.from(existingSlugs).join(', ') || 'None'}]
    
    🧮 STRICT MACRO MATH RULES (CRITICAL - READ CAREFULLY) 🧮:
    - You are a clinical nutritionist. You MUST calculate caloriesPerServing, proteinGrams, carbsGrams, and fatGrams individually for EACH recipe based on its specific ingredients and gram quantities.
    - Recipe 1 and Recipe 2 MUST HAVE DIFFERENT MACROS. Do not just copy-paste the same numbers.
    - Example: If Recipe 1 has 100g Egg (155 cal) and Recipe 2 has 150g Egg (232 cal), their total calories MUST be different.
    - Vary the servingSizeGrams slightly between recipes (e.g., 280g, 310g, 340g) to naturally create different macro totals.
    - Even if both recipes are for the same meal slot (e.g., BREAKFAST), their exact macro numbers MUST be unique.
    
    ⚠️ ANTI-LAZY RULES (VIOLATIONS WILL CAUSE REJECTION) ⚠️:
    ...
    - DO NOT output identical macros across recipes in this batch.
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          { role: 'system', content: 'You are a master commercial database architect and clinical nutritionist for FitFuel. Output completely raw JSON. Never use Markdown blocks. Calculate all macros mathematically based on ingredients.' },
      ...
      // 🛡️ Pass requested slots to the Quality Gate
...
// ─────────────────────────────────────────────────────────────────────────────
// 7. TYPESCRIPT FILE ASSEMBLY & FORMATTING
// ─────────────────────────────────────────────────────────────────────────────
function buildRecipesTs(recipes: z.infer<typeof RecipeSchema>[], dietVariant: string, planCategory: string) {
...
async function writeSeedFile(plan: PlanConfig, recipes: any[], missingItems: any[]) {
  const missingItemsTs = missingItems.map(i => `  { name: ${JSON.stringify(i.name)}, category: ${JSON.stringify(i.category)}, per100Calories: ${i.per100Calories}, per100Protein: ${i.per100Protein}, per100Carbs: ${i.per100Carbs}, per100Fat: ${i.per100Fat}, per100Fiber: ${i.per100Fiber} }`).join(',\n');
...
async function main() {
  ...
  console.log(`\n🚀 MASTER PRODUCTION SEED GENERATOR (STRICT QUALITY GATE)`);
  console.log(`📋 Plans to process: ${queue.length} | Batch Size: ${BATCH_SIZE} (Anti-Lazy Mode)\n`);
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    try {
      ...
      for (let bi = 0; bi < batches.length; bi++) {
        ...
        result.missingFoodItems.forEach(item => { allMissing.set(item.key, item); knownFIKeys.add(item.key); });
        result.recipes.forEach(r => { allRecipes.push(r); usedSlugs.add(r.slug); });
