// lib/activate-digital-plan.ts  (Phase 13D — revised: self-contained, called from PayU success)
// Creates an isDigital UserActivePlan (no delivery) + records coupon redemption. Idempotent.
import { prisma } from "@/lib/prisma";
import { PlanDuration } from "@prisma/client";

const DUR_DAYS: Record<string, number> = {
  TRIAL_DAY: 1, WEEKLY: 7, BI_WEEKLY: 14, MONTHLY_EXCL_WEEKENDS: 26,
  ONE_MONTH: 30, TWO_MONTH: 60, THREE_MONTH: 90,
};

export interface ActivateDigitalArgs {
  orderId: string;
  mealPlanId: string;
  durEnum: string;   // PlanDuration value, e.g. "ONE_MONTH"
}

export async function activateDigitalPlan({ orderId, mealPlanId, durEnum }: ActivateDigitalArgs) {
  ...
  const activePlan = await (prisma as any).userActivePlan.create({
    data: {
      ...
      isDigital: true,                       // <-- delivery cron filters these OUT
      duration: durEnum as PlanDuration,
  ...
  if (order.couponCode && order.discountRs > 0) {
    ...
    if (coupon) {
      await (prisma as any).couponRedemption.create({
        data: { couponId: coupon.id, userId: order.userId, orderId, amountRs: order.discountRs },
      });
    }
  }
  return activePlan;
